...
// XXX Changed from original file
// - Disabled TF_DEBUG_CHECKS - which caused compile error
// I wanted the ability to check if a file exists
...
void tfTraverse( const char* path, tfCallback cb, void* udata )
{
	...
	int tfReadFile( tfDIR* dir, tfFILE* file )
	{
		...
		file->size = (size_t)file->info.st_size;
		tfGetExt( file );
